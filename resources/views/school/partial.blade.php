...
<span class="btn-group float-right" role="group" aria-label="Button group with nested dropdown">
    <a class="btn btn-light" href="{{ route('schools.index') }}" role="button" title="List"><span class="mdi mdi-format-list-bulleted-square"></span></a>
    @unless (Route::is('*.create'))
        <a class="btn btn-light" href="{{ route('schools.create') }}" role="button" title="Create New"><span class="mdi mdi-pencil-plus"></span></a>
    @endunless
</span>
...
<form action="{{ route('schools.'.$action, ['region' => $region->id]) }}" method="POST">
    @csrf
    @if (Route::is('*.edit'))
        @method('PUT')
    @endif
